...
const SenafixProductGrid = () => {
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-24 md:py-32 px-4 sm:px-6">
      {/* Container switched to CSS Grid for easier responsiveness */}
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-28 md:gap-y-32 gap-x-6 lg:gap-x-10 mt-12 md:mt-0">
        {products.map((product, index) => {
          ...
          return (
            <div key={index} className="flex flex-col">
              ...
              <div className="relative bg-white rounded-[2rem] md:rounded-[3rem] shadow-xl shadow-slate-200/60 pt-24 md:pt-28 pb-8 md:pb-10 px-6 md:px-8 flex flex-col h-full border border-slate-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-slate-300/70">
                
                {/* Floating Image Section - Adjusted positioning and size for small screens */}
                <div className="absolute -top-16 md:-top-24 left-1/2 -translate-x-1/2 w-40 h-40 md:w-52 md:h-52 flex items-center justify-center">
                  <div className={`absolute inset-0 bg-gradient-to-br ${product.color} opacity-20 rounded-full blur-2xl md:blur-3xl`}></div>
                  <img 
                    src={product.image} 
                    alt={product.title}
                    className="relative z-10 w-full h-full object-contain drop-shadow-[0_15px_25px_rgba(0,0,0,0.15)] transform transition-transform duration-500 hover:scale-110"
                  />
                </div>
                ...
                <div className="text-center mb-6">
                  ...
                  <div className="inline-block bg-green-50 text-green-600 text-2xl md:text-3xl font-black px-5 md:px-6 py-2 rounded-xl md:rounded-2xl">
                    {product.price}
                  </div>
                </div>
                ...
                {/* WhatsApp Link Button - Scaled down padding for mobile */}
                <a 
                  href={whatsappUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full bg-[#E69B56] hover:bg-[#d48a47] text-white font-bold py-4 md:py-5 rounded-2xl md:rounded-3xl shadow-lg shadow-orange-200/50 active:scale-[0.95] transition-all flex items-center justify-center gap-2 no-underline text-sm md:text-base"
                >
